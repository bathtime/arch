...
unmount_disk () {
	...
	if [[ $(mount | grep -E $disk$bootPart | grep -E "on $mnt$boot_path") ]]; then
		echo "Unmounting $mnt$boot_path..."
		umount -n -R $mnt$boot_path
		sleep .1
...
create_partitions () {
	...
	parted -s $disk mklabel gpt \
			...
			mkpart BOOT ext4 512Mib 2048Mib \
			mkpart SWAP linux-swap 2048Mib 10Gib \
			set $swapPartNum swap on \
			
			#mkpart ROOT ext4 10Gib 100%
			#mkpart ROOT $fstype 10Gib 100%

	if [ "$fstype" = "bcachefs" ]; then
		parted -s $disk mkpart ROOT ext4 10Gib 100%
	else
		parted -s $disk mkpart ROOT $fstype 10Gib 100% 
	fi
