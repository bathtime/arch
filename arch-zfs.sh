...
fstype='zfs'			# btrfs,ext4,bcachefs,f2fs,xfs,jfs,nilfs2
...
create_partitions () {
	...
	read -p "Manually add solaris partition (bf00) with gdisk and hit <ENTER> to continue."
	case $fstype in

		zfs)		check_pkg zfs-utils
				zpool create -f -o ashift=12 -O acltype=posixacl -O relatime=on -O dnodesize=auto -O normalization=formD -O mountpoint=none -O canmount=off -O devices=off -R $mnt zroot $disk$rootPart
				zpool status
				...
				zpool export zroot
				zpool export zroot
				zpool import -d $disk$rootPart $mnt zroot -N
